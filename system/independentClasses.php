...
{
    var $mimeBasedTye = array(
        //Compressed Files
        'application/x-7z-compressed' => 'Compressed File',
        'application/x-rar-compressed' => 'Compressed File',
        'application/zip' => 'Compressed File',

        //Publications
        'image/vnd.djvu' => 'Publication Document',
        'application/epub+zip' => 'Publication Document',
        'application/pdf' => 'Publication Document',
        ...
        'application/vnd.ms-powerpoint' => 'Powerpoint Show',
    ...
    function __construct($supported_file_types = null, $destination = null, $fileSizeLimit = null)
    {
        if ($supported_file_types) $this->setSupportedFileTypes($supported_file_types);

        if ($destination) $this->setDestination($destination);
        else $this->setDestination(_path('uploads', 'absolute').'/');
        
        if ($fileSizeLimit) $this->setFileSizeLimit($fileSizeLimit);
        else $this->setFileSizeLimit('5242880‬');
    }
    function setSupportedFileTypes($supported_file_types)
    {
        $this->supported_file_types = $supported_file_types;
    }
    ...
    function isSupportedFile($file, $ext = null)
    {
        $ext = !$ext ? getExtension($file) : $ext;
        $mimeType = getMimeType($file);

        if (!$mimeType) return false;

        $this->lastExt = strtolower($ext);
        $this->lastMime = strtolower($mimeType);

        if (isset($this->supported_file_types[$ext])) {
            if (is_array($this->supported_file_types[$ext])) {
                foreach ($this->supported_file_types[$ext] as $i => $v) {
                    if ($v == $mimeType) return true;
                }
                return false;
            } else {
                if ($this->supported_file_types[$ext] == $mimeType) return true;
                else return false;
            }
        } else return false;
    }
    function upload_file($field, $at, $supported_ext = null){
        $ret = array();
    
        if($field['error'] == 1 || $field['error'] == 2){
            $ret['error'][] = 'File size of the uploaded file is more than max upload file size.';
            }
        elseif($field['error'] == 3){
            $ret['error'][] = 'File uploaded partially, try again.';
            }
        elseif($field['error'] == 4){
            $ret['error'][] = 'File wasn\'t uploaded, try again.';
            }
    
        if($ret['error']) return $ret;
        
        $target_dir = $this->destination . "/";
        $target_file = $target_dir . basename($field["name"]);
        $fileinfo = pathinfo($target_file);
        $target_file = $target_dir . str_replace(' ', '_', $fileinfo['filename']) . '_' . time() . '.' . $fileinfo['extension'];
        $fileType = pathinfo($target_file, PATHINFO_EXTENSION);
    
        if(!strlen($field["name"])) $ret['error'][] = 'Select file to upload.';
        if(!$this->isSupportedFile($field['tmp_name'], $fileinfo['extension'])){
            $e = 'File Type is not supported. Only supports ';
            foreach($this->supported_file_types as $i=>$v){
                $e .= '.'.$i.', ';
                }
            $ret['error'][] = rtrim($e,', ');
            }
        if ($this->fileSizeLimit && $field['size'] > $this->fileSizeLimit)
            $ret['error'][] = $field["size"].' Bytes is more than supported file size '.$this->fileSizeLimit.' Bytes';
    
        if(!$ret['error']) {
            if (!move_uploaded_file($field["tmp_name"], $target_file)) {
                $ret['error'][] = 'File was not uploaded, please try again.';
                }
            else {
                $fileinfo = pathinfo($target_file);
                $ret['success'] = $fileinfo['basename'];
                $ret['ext'] = $this->lastExt;
                $ret['mime'] = $this->lastMime;
                $ret['type'] = $this->mimeBasedTye[$this->lastMime];
                }
            }
        return $ret;
        }
...
{
    custom_content_type::registerCustomContentTypes();
}
...
{
    page_templates::registerPageTemplates();
}
...
function registerPublicRouters()
{
    public_routers::registerPublicRouters();
}
...
abstract class email_templates
{
    ...
    {
        return $this->get_email_body($dataArray);
        send_mail($to, $subject, $this->get_email_body($dataArray), $toName);
    }
}
function registerEmailTemplates()
{
    email_templates::registerEmailTemplates();
}
...
function registerContactMessageTypes()
{
    contact_message_types::registerContactMessageTypes();
}
...
class minify_handler
{
    ...
    static function renderMinifiedCss($filename, $nextVersion = 0, $printData = false)
    {
        ...
        $targetFileMin = $target . $filename . '_' . $nextVersion . '.min.css';
        $targetFileMinRelative = convertPaths($target) . $filename . '_' . $nextVersion . '.min.css';
...
class permission_manager
{
    ...
    function init()
    {
        ...
        if (HAS_USER()) {
            ...
            if ($_config['user']['roles_list']) {
                $roles = $_config['user']['roles_list'];
 ...
 * */
    ...
    {
        /*
    * @param string $msg activity message
    * @param string $status success|error
    * @param string $type create|read|update|delete|login|logout|password_reset_request
    * @param string $url url to activity
            ...
            if ($config['type'] == 'panel_start') {
                ...
                ?>
                 ...
                 foreach($config['data']['static'] as $i=>$v){
                    ...
                    $field .= '<label>' . $config['label'] . '</label>';
                    $field .= '<select ' . $config['attributes'] . ' class="form-control" id="select_holder_' . $id . '" name="' . $f_name . '">';
