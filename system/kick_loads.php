...
if(file_exists(_path('root','absolute').'/db.ini')){
    $db_settings = parse_ini_file(_path('root','absolute').'/db.ini', true);
    if($project_settings) $project_settings['db'] = $db_settings;
    else $project_settings = array('db' => $db_settings);
    }
...
if(getProjectSettings('db')){
    ...
    $allRoles = $devdb->get_results("SELECT pk_role_id, role_slug FROM dev_user_roles");
   
    if($allRoles){
        foreach($allRoles as $role){
            define('ROLE_'.str_replace('-','_',$role['role_slug']).'_ID', $role['pk_role_id']);
        }
    }
