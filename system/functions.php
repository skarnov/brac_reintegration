...
function checkPermission($edit, $addP, $editP) {
    if ($edit)
        return has_permission($editP);
    else
        return has_permission($addP);
}
...
function autoCompleteSubmitDataToRefillData($ids, $labels, $toJSON = true) {
    ...
    if ($ids) {
        if (is_array($ids)) {
            ...
            array_push($output, array('id' => $ids, 'label' => $labels));
    }
...
function sql_order_by($param, $default_col = null, $default_order = null) {
    ...
    if ($param['order_by']) {
        ...
        if ($param['multiple_order_by'])
            $param['multi_order_by'] = $param['multiple_order_by'];
...
function sql_condition_maker($loop_condition = array(), $param = array()) {
    ...
    if ($COMBINED_LIKE) {
        ...
        if ($each_condition)
            $condition .= " AND ( " . implode(' ' . $COMBINED_LIKE_OPERATOR . ' ', $each_condition) . " )";
    }
...
function print_date($date, $break_date_time = false, $time = true, $addSymbol = false) {
    ...
    if ($break_date_time)
        return $date_output . '<br />' . $time_output;
    else
...
function get_time_ago($time) {
    ...
    if ($get_interval['y'] > 0 || $get_interval['m'] || $get_interval['d'] > 0)
        return 'on ' . print_date($time, false, false);
    elseif (($get_interval['d'] == 0) && ($get_interval['h'] >= 2))
        return $get_interval['h'] . ' hours ago';
    elseif (($get_interval['h'] >= 1) && ($get_interval['h'] < 2))
        return 'About an hour ago';
    elseif (($get_interval['h'] == 0) && ($get_interval['i'] >= 2))
        return $get_interval['i'] . ' minutes ago';
    elseif (($get_interval['i'] >= 1) && ($get_interval['i'] < 2))
        return 'About a minute ago';
    elseif (($get_interval['i'] == 0) && ($get_interval['s'] <= 60))
...
function _checkDate($date, $separator = '-', $format = 'Y-m-d') {
    ...
    if ($formatParts[0] == 'Y')
        $y = $dateParts[0];
    elseif ($formatParts[0] == 'm')
        $m = $dateParts[0];
    elseif ($formatParts[0] == 'd')
        $d = $dateParts[0];

    if ($formatParts[1] == 'Y')
        $y = $dateParts[1];
    elseif ($formatParts[1] == 'm')
        $m = $dateParts[1];
    elseif ($formatParts[1] == 'd')
        $d = $dateParts[1];

    if ($formatParts[2] == 'Y')
        $y = $dateParts[2];
    elseif ($formatParts[2] == 'm')
        $m = $dateParts[2];
    elseif ($formatParts[2] == 'd')
        $d = $dateParts[2];
...
function _checkDateTime($dateTime, $dateFormat = 'Y-m-d', $separator = '-') {
    ...
    if (!_checkDate($date, $separator, $dateFormat))
        return false;
    if (!_checkTime($time))
        return false;
...
function formatNumber($number, $only_number = false, $only_specifier = false) {
    ...
    if ($only_number)
        return $num;
    elseif ($only_specifier)
        return $specifier;
    else
        return $num . $specifier;
}
...
function handleIndexEmptyStrings($data, $index, $handle = 'N/A') {
    if (!$data)
        return $handle;
    if (isset($data[$index]))
        return dbReadableString($data[$index], false, $handle);
    else
        return $handle;
}
...
function is_image($file) {
    ...
    if (function_exists('getimagesize')) {
        ...
        if (in_array($data['mime'], $mime_types))
            return true;
        else
            return false;
    } elseif (function_exists('mime_content_type')) {
        $data = mime_content_type($file);
        if (in_array($data, $mime_types))
            return true;
        else
            return false;
    }
...
function zip_extractor($file, $path = null, $fileNameAsFolder = true, $delete_zip = true) {
    ...
    if (file_exists($file) === false)
        $ret['error'][] = 'Given file doesn\'t exist';
    ...
    if (!isset($ret['error']) || !$ret['error']) {
        ...
        if ($res === TRUE) {
            ...
            $ret['error'][] = 'Failed to open the ZIP file';
    }
    ...
    if ($dependency[$pointer]) {
        foreach ($dependency[$pointer] as $i => $v) {
            if (isset($dependency[$v])) {
                if (!_process_toposort($v, $dependency, $order, $pre_processing, $reportError))
                    return false;
            }
            if (!in_array($v, $order))
                $order[] = $v;
...
function _topological_sort($data, $dependency, &$reportError = null) {
    ...
    foreach ($data as $i => $v) {
        if (!_process_toposort($i, $dependency, $order, $pre_processing, $reportError))
            return false;
    }
    return $order;
}
...
function pagination($total, $per_page, $start, $items = 5, $style = 'table_footer') {
    ...
    if ($this_page > $total_num_pages)
        return '';

    if ($this_page - 2 > 0)
        $start_from = $this_page - 2;
    else
        $start_from = 1;

    if ($this_page + 2 <= $total_num_pages)
        $start_to = $this_page + 2;
    else
        $start_to = $total_num_pages;
    ...
    if ($style == 'basic') {
        ...
        if ($start_from > 1)
            $_data .= '<li><a href="' . build_url(array('start' => ($start_from - 2))) . '">&laquo;</a></li>';
        ...
        if ($start_to < $total_num_pages)
            $_data .= '<li><a href="' . build_url(array('start' => ($i - 1))) . '">&raquo;</a></li>';
        ...
        if ($start_from > 1)
            $_data .= '<li class="paginate_button previous"><a href="' . build_url(array('start' => ($start_from - 2))) . '">Previous</a></li>';
        ...
        if ($start_to < $total_num_pages)
            $_data .= '<li class="paginate_button next"><a href="' . build_url(array('start' => ($i - 1))) . '">Next</a></li>';
...
function filterForm($form_config = array(), $pageVars = array(), $data = array()) {
    ...
    <div class="panel">
        ...
        <div class="panel-body p5 pb15 pt15">
            <form class="" role="form" name="filter_form" method="get" action="">
    <?php
...
function processToRender($data, $language = null, $strictCheck = false) {
    ...
    if ($ret)
        $outputData = isset($matches[1]) ? $matches[1] : '';
...
function get_countries() {
    global $devdb;
    $result = $devdb->get_results("SELECT id, nicename FROM countries");
    return $result;
}

function get_cities($country_id) {
    global $devdb;
    $result = $devdb->get_results("SELECT id, name FROM cities WHERE fk_country_id = '$country_id'");
    return $result;
}
...
function get_policeStation($district_id) {
    global $devdb;
    $result = $devdb->get_results("SELECT id, name FROM bd_police_stations WHERE district_id = '$district_id'");
    return $result;
}

function get_postOffice($district_id) {
    global $devdb;
    $result = $devdb->get_results("SELECT id, name FROM bd_post_offices WHERE district_id = '$district_id'");
    return $result;
}

function get_municipality($district_id) {
    global $devdb;
    $result = $devdb->get_results("SELECT id, name FROM bd_municipalities WHERE district_id = '$district_id'");
    return $result;
}

function get_cityCorporation($district_id) {
    global $devdb;
    $result = $devdb->get_results("SELECT id, name FROM bd_city_corporations WHERE district_id = '$district_id'");
    return $result;
}
...
function joinFiles(array $files, $data) {
    if (!is_array($files)) {
