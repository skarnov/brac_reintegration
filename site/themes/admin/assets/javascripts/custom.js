...
var in_page_add_event = function(option) {
    ...
    var ths = option.ths ? option.ths : null;
    var url = option.url;
    var success_message = typeof option.success_message !== "undefined" ? option.success_message : null;
    var show_submit_btn = typeof option.show_submit_btn !== "undefined" ? option.show_submit_btn : true;
    ...
    thisObject.next_step = function() {
        ...
        if (show_submit_btn) dialogOptions.buttons['Add'] = submitBtnOptions;
        var this_dialog = the_form_container.dialog(dialogOptions);
    };
    ...
    if (edit_mode) {
        ...
        basicAjaxCall({
            beforeSend: function() {
                if (ths) show_button_overlay_working(ths);
                showLoading()
            },
            complete: function() {
                if (ths) hide_button_overlay_working(ths);
                hideLoading()
            },
            url: edit_form_url,
            data: data,
...
function _month_year_picker(id, args) {
    var elm = $('#' + id);
    var elm_wrapper = elm.parent();
    var options = {
        format: "yyyy-mm",
...
function _month_picker(id, args) {
    var elm = $('#' + id);
    var elm_wrapper = elm.parent();
    var options = {
        format: "mm",
...
function datepickerCalculate(id, args) {
    var elm = $('#' + id);
    var elm_wrapper = elm.parent();
    var options = {
        todayBtn: "linked",
...
function _timepicker(id, options) {
    if (typeof options === 'undefined') options = {}

    var defaultOptions = {
        'add_calendar_icon': true,
        'add_clear_icon': true,
    };
    ...
    if (!elm_wrapper.hasClass('input-group')) elm_wrapper.addClass('input-group');
    elm_wrapper.find('.input-group-addon').remove();
    if (options.add_calendar_icon)
        elm_wrapper.append('<span class="input-group-addon the_calendar_icon"><i class="fa fa-calendar"></i></span>');
    else elm_wrapper.addClass('no_calendar');

    if (options.add_clear_icon)
        elm_wrapper.append('<span class="input-group-addon date_clear clear_date"><i class="text-danger fa fa-times"></i></span>');
    else elm_wrapper.addClass('no_clear');
}
/* ---------------- */
...
$('.verifyUnique').on('keyup, focusin', function(e) {
    var ths = $(this);
    var container = ths.closest('.form-group');
    container.find('.input-group-addon').html('&nbsp;');
    container.removeClass('has-error');
    container.find('.help-block').html('');
    ths.attr('data-verified', 'no');
    ths.closest('form').find('input:submit, button:submit').prop('disabled', true);
    ths.closest('form').find('#checkNext').addClass('disabled');
    ths.closest('form').find('#checkNext').html('Verifying Data...');
});

$('.verifyUnique').on('focusout', function(e) {
    var ths = $(this);
    var thsVal = ths.val();
    ...
    ths.attr('data-verified', 'no');
    var submitBtn = ths.closest('form').find('input:submit, button:submit');
    var subumitBtnText = submitBtn.html();
    ...
    basicAjaxCall({
        beforeSend: function() {
            ...
            submitBtn.html('<i class="fa fa-cog fa-spin"></i>Verifying Data');
            show_button_overlay_working(submitBtn);
        },
        complete: function() {
            hide_button_overlay_working(ths);
            submitBtn.html(subumitBtnText);
            hide_button_overlay_working(submitBtn);
        },
        data: data,
        success: function(ret) {
            if (ret === '1') {
                symbolContainer.html('<i class="fa fa-check text-success"></i>').show();
                ths.attr('data-verified', 'yes');
                ths.closest('form').find('input:submit, button:submit').prop('disabled', false);
                $.growl.notice({ title: '', message: 'Data verified successfully' });
                ths.closest('form').find('#checkNext').removeClass('disabled');
                ths.closest('form').find('#checkNext').html('<i class="fa fa-cog btn-label"></i>Check & Proceed');
            } else if (ret === '0') {
                container.addClass('has-error');
                helpContainer.html(messageError);
                symbolContainer.html('<i class="fa fa-exclamation-triangle text-danger"></i>').show();
                ths.closest('form').find('input:submit, button:submit').prop('disabled', true);
                $.growl.error({ title: 'Error', message: messageError });
            }
        }
    });
});