...
function get_image(image, size) {
    ...
    return _root_path_ + '/api/dev_administration/get_image?' + query;
}
...
var set_autosuggest = function(options) {
     ...
     * on_remove: function() to call when an item has been removed
    ...
    ths.remove_item = function(closeBtn) {
        ...
        if (ths.options.on_remove) ths.options.on_remove();
    };
    ...
    ths.init = function() {
        ...
        ths.input_field.autocomplete({
            ...
            source: function(request, response) {
                ths.current_term = request.term; //extractLast(request.term);
                $.ajax({
                    ...
                    type: 'post',
                    data: {
                        term: request.term, //extractLast(request.term),
                        internalToken: _internalToken_,
                        _data: ths.options
                    },
                    cache: false,
            ...
            search: function() {
                var term = this.value; //extractLast(this.value);
...
function getTableColumnWidths(table, width_unit) {
    ...
    var selector = 'th'
    if (table.find('>thead>tr').length) {
        container = table.find('>thead>tr');
        selector = 'th'
    } else if (table.find('>thead>th').length) {
        container = table.find('>thead');
        selector = 'th'
    } else if (table.find('>tbody>tr').length) {
        container = table.find('>tbody>tr');
        selector = 'td'
    } else if (table.find('>tr').length) {
