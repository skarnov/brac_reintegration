...
class dev_administration{
    ...
    function init(){
        ...
        apiRegister($this,'get_image');
        apiRegister($this,'get_responsive_image');
    ...
    function get_responsive_image(){
        ...
        $finalSize = array();
        preg_match('/cropped\/([0-9]+x[0-9]+)/', $image, $finalSize);
        if(isset($finalSize[1]))
            @list($ret['w'], $ret['h']) = explode('x', $finalSize[1]);
    ...
    function adm_menus(){
        ...
        if(has_permission('manage_user_activities')) $ret = admenu_register($params);
        ...
        if(has_project_settings('admin_menu,system_settings') && has_permission('manage_system_settings')) admenu_register($params);
