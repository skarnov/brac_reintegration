    ...
    function set_role_form($param = array()){
        ...
        if(!has_permission('manage_roles')){
            ...
            if($is_update && !$fillData)
                ...
                if(!$is_update){
                    $param['role_slug'] = form_modifiers::slug($param['role_name']);
                    $temp = form_validator::unique($param['role_slug'],'dev_user_roles','role_slug');
                    if($temp !== true) $param['role_slug'] = $temp;
                } else {
                    $param['role_slug'] = $fillData['role_slug'];
                }
                
                if(!$ret['error']){
                    $data = array(
                        'role_slug' => $param['role_slug'],
