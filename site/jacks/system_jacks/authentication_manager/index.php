		...
		if(!$ret['error']){
            ...
            if($ret){
                ...
                elseif($ret['user_status'] == 'not_verified'){
                    $ret['error'][] = 'Your account is not verified.';
                    ...
                    $verified = verify_password($data['user_password'], $ret['user_password']);
                    if(!$verified)
                        $ret['error'][] = "Mismatched Email or Password.";
                    else{
...
function validateEmail($email){
    ...
    $temp = form_validator::unique($email,'dev_users','user_email', " user_email_verified = '1'");
    if($temp !== true)
...
function getEmailResetCode(){
    $codeOkay = false;
    $code = hash_password('3devsIT'.mt_rand(10000,99999).date('Y-m-d H:i:s'));
    global $devdb;

    for($i=0;$i<5;$i++){
        $sql = "SELECT * FROM dev_users WHERE user_email_verification_code = '$code' LIMIT 0,1";
        $found = $devdb->get_row($sql);
        if(!$found){
            $codeOkay = true;
            break;
...
function processLoginAjax(){
    ...
    $ret = array();
    if(isset($_POST['login_request'])){
        $ret = $authManager->perform_login($_POST['data']);
        if(!$ret['error']){
            $ret['success'] = 1;
            if($_config['user'] && $_config['user']['user_type'] == 'admin'){
                $_SESSION['admin_loggedin'] = 1;
...
function verifyEmail(){
    ...
    $profile_manager = jack_obj('dev_profile_management');
    if(isset($_GET['key'])){
        $sql = "SELECT * FROM dev_users WHERE user_status = 'not_verified' AND user_email_verified = '0' AND user_email_verification_code = '".$_GET['key']."'";
        $found = $devdb->get_row($sql);

        if($found){
            $sql = "UPDATE dev_users SET user_email_verification_code = null, user_email_verified = '1', user_status = 'active' WHERE pk_user_id = '".$found['pk_user_id']."'";
            $devdb->query($sql);
            $profile_manager->reCacheUser($found['pk_user_id']);

            //find other accounts with this email which are unverified,
            //delete those accounts

            $sql = "SELECT pk_user_id FROM dev_users WHERE user_email = '".$found['user_email']."' AND user_email_verified = 0";
            $fakeUsers = $devdb->get_results($sql);

            if($fakeUsers){
                foreach($fakeUsers as $i=>$v){
                    $sql = "DELETE FROM dev_users WHERE pk_user_id = '".$v['pk_user_id']."'";
                    $deleteFakeUser = $devdb->query($sql);
                    $profile_manager->reCacheUser($v['pk_user_id']);
                    }
                }

            $publicSignIn = getProjectSettings('public_theme,public_login_page');
            add_notification('Thank you for verifying your email address. You can now login with your login information','success');
            header('Location:'.url($publicSignIn));
            exit();
            }
        else{
            header('Location:'.url(''));
