...
$filter_district = $_GET['district'] ? $_GET['district'] : 'Jashore';
$filter_sub_district = $_GET['sub_district'] ? $_GET['sub_district'] : 'Abhaynagar';
...
$results = $this->get_project_division_district_upazila_date_reports($args);
...
$count_results = $this->get_project_division_district_upazila_date_reports($args);
...
if ($_GET['download_csv']) {
    unset($args['limit']);
    $data = $this->get_project_division_district_upazila_date_reports($args);
    $data = $data['data'];
    ...
    $csvFile = $csvFolder . 'Project_Division_District_Upazila_Date_Wise_MIS_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Project, Division, District, Upazila and Date Wise MIS Report', '');
...
<div class="page-header">
    <h1>Project, Division, District, Upazila and Date MIS Reports</h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            <?php
            echo linkButtonGenerator(array(
                'href' => '?download_csv=1&project=' . $filter_project . '&division=' . $filter_division . '&district=' . $filter_district . '&sub_district=' . $filter_sub_district . '&entry_start_date=' . $filter_entry_start_date . '&entry_end_date=' . $filter_entry_end_date,
...
<div class="table-primary table-responsive">
    ...
    <div class="table-header">
        <?php echo searchResultText($results['total'], $start, $per_page_items, count($results['data']), 'Project, Division, District, Upazila and Date MIS Report') ?>
    </div>
