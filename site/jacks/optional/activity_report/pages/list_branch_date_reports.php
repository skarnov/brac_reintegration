...
$filter_branch = $_GET['branch'] ? $_GET['branch'] : 2;
...
$results = $this->get_branch_date_reports($args);
...
$count_results = $this->get_branch_date_reports($args);
...
if ($_GET['download_csv']) {
    unset($args['limit']);
    $data = $this->get_branch_date_reports($args);
    $data = $data['data'];
    ...
    $csvFile = $csvFolder . 'Branch_and_Date_Wise_MIS_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Branch and Date Wise MIS Report', '');
...
<div class="page-header">
    <h1>Branch and Date MIS Reports</h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            <?php
            echo linkButtonGenerator(array(
                'href' => '?download_csv=1&branch=' . $filter_branch . '&entry_start_date=' . $filter_entry_start_date . '&entry_end_date=' . $filter_entry_end_date,
...
<div class="table-primary table-responsive">
    ...
    <div class="table-header">
        <?php echo searchResultText($results['total'], $start, $per_page_items, count($results['data']), 'Branch and Date MIS Report') ?>
    </div>
