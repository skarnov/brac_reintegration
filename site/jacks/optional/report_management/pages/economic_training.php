...
$args = array(
    ...
    'select_fields' => array(
        ...
        'create_date' => 'dev_economic_training.create_date',
    ...
    'order_by' => array(
        'col' => 'dev_economic_training.fk_customer_id',
        'order' => 'DESC'
    ),
);
...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Economic_Training_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Economic Training Report', '');
    ...
    $headers = array(
        ...
        'Direct Training - Institution Name',
        'Direct Training - Place of Training',
        'Direct Training - Training Start Date',
        'Direct Training - Training End Date',
        'Direct Training - Certificate Received',
        'Direct Training - Comment',
        'Referral Training Type',
        'Referral Training - Institution Name',
        'Referral Training - Place of Training',
        'Referral Training - Training Start Date',
        'Referral Training - Training End Date',
        'Referral Training - Certificate Received',
        'Referral Training - Comment',
    );
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , ucfirst($value['training_type'])
                , $value['direct_training_type'] . ' ' . $value['other_direct_training_type']
                , $value['training_institution_name']
                , $value['training_place']
                , $value['economic_training_start_date'] && $value['economic_training_start_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['economic_training_start_date'])) : 'N/A'
                , $value['economic_training_end_date'] && $value['economic_training_end_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['economic_training_end_date'])) : 'N/A'
                , ucfirst($value['is_certification_received'])
                , $value['economic_training_comment']
                , $value['referral_training_type'] . ' ' . $value['other_referral_training_type']
                , $value['referral_training_institution_name']
                , $value['referral_training_place']
                , $value['referral_economic_training_start_date'] && $value['referral_economic_training_start_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['referral_economic_training_start_date'])) : 'N/A'
                , $value['referral_economic_training_end_date'] && $value['referral_economic_training_end_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['referral_economic_training_end_date'])) : 'N/A'
                , ucfirst($value['referral_certification_received'])
                , $value['referral_economic_training_comment']
            );
...
<div class="page-header">
    <h1>Economic Training Report</h1>
    <div class="oh">
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'Economic Training') ?>
    </div>
        ...
        <tbody>
            ...
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td class="text-capitalize"><?php echo $case['training_type']; ?></td>
                </tr>
                <?php
            }
            ?>
