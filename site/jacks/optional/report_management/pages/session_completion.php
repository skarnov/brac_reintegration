...
$args = array(
    ...
    'select_fields' => array(
        ...
        'is_completed' => 'dev_psycho_completions.is_completed',
        'dropout_reason' => 'dev_psycho_completions.dropout_reason',
        'review_session' => 'dev_psycho_completions.review_session',
        'client_comments' => 'dev_psycho_completions.client_comments',
        'counsellor_comments' => 'dev_psycho_completions.counsellor_comments',
        'final_evaluation' => 'dev_psycho_completions.final_evaluation',
        'required_session' => 'dev_psycho_completions.required_session',
    ),
    ...
    'order_by' => array(
        'col' => 'dev_psycho_completions.fk_customer_id',
        'order' => 'DESC'
    ),
);
...
if ($_GET['download_csv']) {
    $args['select_fields'] = array(
        ...
        'entry_date' => 'dev_psycho_completions.entry_date',
        'create_date' => 'dev_psycho_completions.create_date',
        'is_completed' => 'dev_psycho_completions.is_completed',
        'dropout_reason' => 'dev_psycho_completions.dropout_reason',
        'review_session' => 'dev_psycho_completions.review_session',
        'client_comments' => 'dev_psycho_completions.client_comments',
    ...
    $csvFile = $csvFolder . 'Session_Completion_Status_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Session Completion Status Report', '');
    ...
    $headers = array(
        ...
        'Is Completed Counseling Session',
        'Drop-out Reason',
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , ucfirst($value['is_completed'])
                , $value['dropout_reason']
                , $value['review_session']
                , $value['client_comments']
                , $value['counsellor_comments']
                , $value['final_evaluation']
                , ucfirst($value['required_session'])
            );
...
<div class="page-header">
    <h1>Session Completion Status Report</h1>
    <div class="oh">
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'Session Completion') ?>
    </div>
    <table class="table table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <th>Entry Date</th>
                <th>Is Completed Counseling Session</th>
                <th>Review</th>
                <th>Comments of the Client</th>
                <th>Counselor’s Comment</th>
                <th>Final Evaluation</th>
                <th>Required Session After Completion</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td class="text-capitalize"><?php
                        echo $case['is_completed'];
                        $case['dropout_reason'] ? '<br/>Drop-out Reason:' . $case['dropout_reason'] : ''
                        ?></td>
                    <td><?php echo $case['review_session']; ?></td>
                    <td><?php echo $case['client_comments']; ?></td>
                    <td><?php echo $case['counsellor_comments']; ?></td>
                    <td><?php echo $case['final_evaluation']; ?></td>
                    <td class="text-capitalize"><?php echo $case['required_session']; ?></td>
                </tr>
                <?php
            }
            ?>
