...
if ($_GET['download_csv']) {
    $args['select_fields'] = array(
        ...
        'support_date' => 'dev_immediate_supports.entry_date AS support_date',
        'arrival_place' => 'dev_immediate_supports.arrival_place',
        'entry_date' => 'dev_immediate_supports.entry_date',
        'create_date' => 'dev_immediate_supports.create_date',
    );
    unset($args['limit']);
    ...
    $csvFile = $csvFolder . 'Immediate_Support_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Immediate Support Report', '');
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , $value['support_date'] && $value['support_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['support_date'])) : 'N/A'
                , date('d-m-Y', strtotime($value['create_date']))
                , $value['pk_immediate_support_id']
                , $value['customer_id']
                , $value['arrival_place']
                , $value['immediate_support']
            );
...
<div class="page-header">
    <h1>Immediate Support Report</h1>
    <div class="oh">
        ...
        <tbody>
            ...
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td><?php echo $case['arrival_place']; ?></td>
                    <td><?php echo $case['support_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['support_date'])) ?></td>
                    <td><?php echo $case['immediate_support']; ?></td>
                </tr>
                <?php
            }
            ?>
