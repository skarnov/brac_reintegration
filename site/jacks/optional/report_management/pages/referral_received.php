...
$args = array(
    ...
    'order_by' => array(
        'col' => 'dev_economic_referral_received.fk_customer_id',
        'order' => 'DESC'
    ),
);
...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Referral_Received_and_Linkage_Support_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Referral Received and Linkage Support Report', '');
    ...
    $headers = array(
        ...
        'Referral Service Received Date',
        'Comment',
    );
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , $value['referral_id']
                , $value['referred_for']
                , $value['referral_service_provider']
                , $value['received_date'] && $value['received_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['received_date'])) : 'N/A'
                , $value['referral_comment']
            );
...
<div class="page-header">
    <h1> Referral Received and Linkage Support Report</h1>
    <div class="oh">
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'Referral Received and Linkage Support Report') ?>
    </div>
    <table class="table table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <th>Referral ID</th>
                <th>Referral For</th>
                <th>Referral Service Provider</th>
                <th>Referral Service Received Date</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td><?php echo $case['referral_id']; ?></td>
                    <td><?php echo $case['referred_for']; ?></td>
                    <td><?php echo $case['referral_service_provider']; ?></td>
                    <td><?php echo $case['received_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['received_date'])) ?></td>
                </tr>
                <?php
            }
            ?>
