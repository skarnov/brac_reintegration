...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Observation_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Observation Report', '');
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , $value['event_start_date'] && $value['event_start_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['event_start_date'])) : ''
                , $value['event_start_time']
                , $value['event_end_date'] && $value['event_end_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['event_end_date'])) : ''
                ...
                , $value['preparatory_work']
                , $value['time_management']
                , $value['participants_attention']
                , $value['logistical_arrangements']
                , $value['relevancy_delivery']
                , $value['participants_feedback']
                , $value['event_note']
                , $value['user_fullname']
            );
            ...
            <div class="oh">
                <div class="btn-group btn-group-sm">
                    ...
                    echo linkButtonGenerator(array(
                        'href' => '?download_csv=1&name=' . $filter_name . '&project_id=' . $filter_project . '&division=' . $filter_division . '&district=' . $filter_district . '&sub_district=' . $filter_sub_district . '&union=' . $filter_union . '&entry_start_date=' . $filter_entry_start_date . '&entry_end_date=' . $filter_entry_end_date,
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($events['total'], $start, $per_page_items, count($events['data']), 'Observation Report') ?>
    </div>
...
<!--                    <td><?php echo $event['project_short_name'] ?></td>-->
                    ...
                    <?php if ($this->observation_report_pass_mark < $event['observation_score']) : ?>
                        <td style="background-color: green; color:black"><?php echo $event['observation_score']; ?></td>
                    <?php else: ?>
                        <td style="background-color: red; color:black"><?php echo $event['observation_score']; ?></td>
                    <?php endif ?>
                </tr>
                <?php
            }
