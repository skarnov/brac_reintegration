...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Social_Reintegration_Support_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Social Reintegration Support Report', '');
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , $value['customer_id']
                , $value['social_support_entry_date'] && $value['social_support_entry_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['social_support_entry_date'])) : 'N/A'
                , date('d-m-Y', strtotime($value['create_date']))
                , $value['support_referred'] . ' ' . $value['other_support_referred']
                , $value['other_social_protection']
            );
...
<div class="page-header">
    <h1>Social Reintegration Support Report</h1>
    <div class="oh">
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'Social Reintegration Support') ?>
    </div>
    <table class="table table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <th>Support Referred</th>
                <th>Other Social Protection</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td><?php echo $case['social_support_entry_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['social_support_entry_date'])) ?></td>
                    <td><?php echo $case['create_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['create_date'])) ?></td>
                    <td class="text-capitalize"><?php echo $case['support_referred'] . ' ' . $case['other_support_referred']; ?></td>
                    <td><?php echo $case['other_social_protection'] ?></td>
                </tr>
                <?php
            }
            ?>
