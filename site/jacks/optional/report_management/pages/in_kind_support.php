...
$args = array(
    ...
    'order_by' => array(
        'col' => 'dev_economic_inkind.fk_customer_id',
        'order' => 'DESC'
    ),
);
...
if ($_GET['download_csv']) {
    $args['select_fields'] = array(
        ...
        'entry_date' => 'dev_economic_inkind.entry_date',
        'create_date' => 'dev_economic_inkind.create_date',
        'in_kind_type' => 'dev_economic_inkind.in_kind_type',
        'support_delivery_date' => 'dev_economic_inkind.support_delivery_date',
        'inkind_project' => 'dev_economic_inkind.inkind_project',
        'other_inkind_project' => 'dev_economic_inkind.other_inkind_project',
    ...
    $csvFile = $csvFolder . 'In_Kind_Support_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('In Kind Support Report', '');
    ...
    $headers = array(
        ...
        'Support Received From - Organization Name',
        'Support Received From - Support Delivery Date',
        'Support Received From - Support Type',
        'Support Amount',
        'Description of Support Delivered',
        'Type of Business',
        'Comments',
    );
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , ucfirst($value['in_kind_type'])
                , $value['organization_name']
                , $value['support_delivery_date'] && $value['support_delivery_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['support_delivery_date'])) : 'N/A'
                , $value['inkind_project'] . ' ' . $value['other_inkind_project']
                , $value['support_amount']
                , $value['economic_support_delivered']
                , $value['business_type'] . ' ' . $value['other_business_type']
                , $value['economic_other_comments']
            );
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'In Kind Support') ?>
    </div>
    <table class="table table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <th>Entry Date</th>
                <th>In Kind Type</th>
                <th>Support Delivery Date</th>
                <th>Support Amount</th>
                <th>Description of Support Delivered</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td class="text-capitalize"><?php echo $case['in_kind_type']; ?></td>
                    <td><?php echo $case['support_delivery_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['support_delivery_date'])) ?></td>
                    <td><?php echo $case['support_amount']; ?></td>
                    <td><?php echo $case['economic_support_delivered']; ?></td>
                </tr>
                <?php
            }
            ?>
