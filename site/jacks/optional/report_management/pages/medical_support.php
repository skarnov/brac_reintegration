...
$args = array(
    ...
    'order_by' => array(
        'col' => 'dev_social_supports.fk_customer_id',
        'order' => 'DESC'
    ),
);
...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Medical_Support_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Medical Support Report', '');
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , $value['customer_id']
                , $value['medical_support_entry_date'] && $value['medical_support_entry_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['medical_support_entry_date'])) : 'N/A'
                , date('d-m-Y', strtotime($value['create_date']))
                , ucfirst($value['medical_support_type'])
                , $value['medical_institution_name']
                , $value['treatment_allowance']
                , $value['treatment_allowance_date'] && $value['treatment_allowance_date'] != '0000-00-00' ? date('d-m-Y', strtotime($value['treatment_allowance_date'])) : 'N/A'
                , $value['treatment_allowance_comment']
            );
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'Medical Support') ?>
    </div>
    <table class="table table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <th>Medical Support Type</th>
                <th>Institution Name (Hospital/Clinic)</th>
                <th>Treatment Allowance</th>
                <th>Support Date</th>
                <th>Comment</th>
            </tr>
        </thead>
        <tbody>
            ...
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td><?php echo $case['medical_support_entry_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['medical_support_entry_date'])) ?></td>
                    <td><?php echo $case['create_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['create_date'])) ?></td>
                    <td class="text-capitalize"><?php echo $case['medical_support_type'] ?></td>
                    <td><?php echo $case['medical_institution_name'] ?></td>
                    <td><?php echo $case['treatment_allowance'] ?></td>
                    <td><?php echo $case['treatment_allowance_date'] == '0000-00-00' ? '' : date('d-m-Y', strtotime($case['treatment_allowance_date'])) ?></td>
                    <td><?php echo $case['treatment_allowance_comment'] ?></td>
                </tr>
                <?php
            }
            ?>
