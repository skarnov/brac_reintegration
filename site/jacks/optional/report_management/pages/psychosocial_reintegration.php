...
$args = array(
    ...
    'order_by' => array(
        'col' => 'dev_psycho_supports.fk_customer_id',
        'order' => 'DESC'
    ),
);
...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Psychosocial_Reintegration_Support_Services_Report-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('Psychosocial Reintegration Support Services Report', '');
    ...
    if ($data) {
        ...
        foreach ($data as $value) {
            $dataToSheet = array(
                ...
                , $value['customer_id']
                , $value['first_meeting'] && $value['first_meeting'] != '0000-00-00' ? date('d-m-Y', strtotime($value['first_meeting'])) : 'N/A'
                , date('d-m-Y', strtotime($value['create_date']))
                , $value['problem_identified'] . ' ' . $value['other_problem_identified']
                , $value['problem_description']
                , $value['initial_plan']
                , ucfirst($value['session_place'])
                , $value['session_number']
                , $value['session_duration']
                , $value['other_requirements']
                , $value['reffer_to']
                , $value['referr_address']
                , $value['contact_number'] . "\r"
                , $value['reason_for_reffer'] . ' ' . $value['other_reason_for_reffer']
            );
...
<div class="page-header">
    <h1>Psychosocial Reintegration Support Services Report</h1>
    <div class="oh">
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($cases['total'], $start, $per_page_items, count($cases['data']), 'Psychosocial Reintegration Support Services') ?>
    </div>
        ...
        <tbody>
            ...
            foreach ($cases['data'] as $i => $case) {
                ...
                <tr>
                    ...
                    <td><?php echo $case['problem_identified'] . ' ' . $case['other_problem_identified'] ?></td>
                    <td><?php echo $case['problem_description'] ?></td>
                    <td class="text-capitalize"><?php echo $case['session_place'] ?></td>
                </tr>
                <?php
            }
            ?>
