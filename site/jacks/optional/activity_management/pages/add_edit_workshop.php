...
if (!checkPermission($edit, 'add_workshop', 'edit_workshop')) {
...
if ($edit) {
    $pre_data = $this->get_workshops(array('id' => $edit, 'single' => true));

    if (!$pre_data) {
        add_notification('Invalid workshop, no data found.', 'error');
...
if ($_POST) {
    ...
    $ret = $this->add_edit_workshop($data);

    if ($ret['success']) {
        $msg = "Workshop has been " . ($edit ? 'updated.' : 'saved.');
        add_notification($msg);
        $activityType = $edit ? 'update' : 'create';
        user_activity::add_activity($msg, 'success', $activityType);
        if ($edit) {
            header('location: ' . url('admin/dev_activity_management/manage_workshops?action=add_edit_workshop&edit=' . $edit));
        } else {
...
<div class="page-header">
    <h1><?php echo $edit ? 'Update ' : 'New ' ?> Workshop</h1>
    <div class="oh">
        ...
        <div class="panel-body">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input id="startDate" type="text" class="form-control" name="workshop_start_date" value="<?php echo $pre_data['workshop_start_date'] && $pre_data['workshop_start_date'] != '0000-00-00' ? date('d-m-Y', strtotime($pre_data['workshop_start_date'])) : ''; ?>">
                    </div>
                    <script type="text/javascript">
                        init.push(function () {
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input id="endDate" type="text" class="form-control" name="workshop_end_date" value="<?php echo $pre_data['workshop_end_date'] && $pre_data['workshop_end_date'] != '0000-00-00' ? date('d-m-Y', strtotime($pre_data['workshop_end_date'])) : ''; ?>">
                    </div>
                    <script type="text/javascript">
                        init.push(function () {
                            _datepicker('endDate');
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="workshop_name" value="<?php echo $pre_data['workshop_name'] ? $pre_data['workshop_name'] : ''; ?>">
                </div>
                <div class="form-group">
                    <label>Duration of Workshop</label>
                    <input type="text" class="form-control" name="workshop_duration" value="<?php echo $pre_data['workshop_duration'] ? $pre_data['workshop_duration'] : ''; ?>">
                </div>
            </div>
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="workshop_venue" value="<?php echo $pre_data['workshop_venue'] ? $pre_data['workshop_venue'] : ''; ?>">
                </div>
                <div class="form-group">
                    <label class="control-label input-label">Workshop Held at</label>
                    <div class="select2-primary">
                        <select class="form-control" name="workshop_held" style="text-transform: capitalize">
                            <option value="">Select One</option>
                            <?php foreach ($this->all_workshop_held as $key => $value) : ?>
                                <option id="<?php echo $key ?>" value="<?php echo $key ?>" <?php echo $pre_data && $pre_data['workshop_held'] == $key ? 'selected' : '' ?>><?php echo $value ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                </div>
