...
$pre_data = array();
if ($edit) {
    $pre_data = $this->get_economic_referrals(array('id' => $edit, 'single' => true));
...
if ($_POST) {
    ...
    $ret = $this->add_edit_economic_referral($data);

    if ($ret['success']) {
        $msg = "Economic Referral Data has been " . ($edit ? 'updated.' : 'saved.');
        add_notification($msg);
        $activityType = $edit ? 'update' : 'create';
        user_activity::add_activity($msg, 'success', $activityType);
        header('location: ' . url('admin/dev_customer_management/manage_cases?action=add_edit_economic_referral&edit=' . $edit . '&customer_id=' . $customer_id . '&case_id=' . $case_id));
        exit();
    } else {
        $pre_data = $_POST;
...
<div class="page-header">
    <h1>Section 4.4: <?php echo $edit ? 'Update ' : 'New ' ?>Referral and Linkage Support</h1>
    <div class="oh">
    ...
    <div class="panel">
        <div class="panel-body">
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        <label>Referred Organization</label>
                        <input class="form-control" type="text" name="referred_organization" value="<?php echo $pre_data['referred_organization'] ? $pre_data['referred_organization'] : ''; ?>">
                    </div>
                    <div class="form-group">
                        <label>Address of Referred Organization</label>
                        <input class="form-control" type="text" name="referred_organization_address" value="<?php echo $pre_data['referred_organization_address'] ? $pre_data['referred_organization_address'] : ''; ?>">
                    </div>
                    <div class="form-group">
                        <label>Referral Date</label>
                        <div class="input-group">
                            <input id="ReferralDate" type="text" class="form-control" name="referral_date" value="<?php echo $pre_data['referral_date'] && $pre_data['referral_date'] != '0000-00-00' ? date('d-m-Y', strtotime($pre_data['referral_date'])) : date('d-m-Y'); ?>">
                        </div>
                        <script type="text/javascript">
                            init.push(function () {
