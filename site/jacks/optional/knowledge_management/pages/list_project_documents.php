...
$project_documents = $this->get_knowledge($args);
$pagination = pagination($project_documents['total'], $per_page_items, $start);
...
$all_project_document_tags = $this->get_lookups('project_document');
            ...
            <div class="oh">
                <div class="btn-group btn-group-sm">
                    ...
                    echo linkButtonGenerator(array(
                        'href' => $myUrl . '?action=add_edit_project_document',
                        'action' => 'add',
                        'icon' => 'icon_add',
                        'text' => 'New Project Document',
        ...
        <div class="col-md-4">
            <div class="stat-row">
                <div class="stat-cell bg-warning">
                    <span class="text-bg"><?php echo $project_documents['total'] ?></span><br>
...
<div class="form-group col-sm-3">
    ...
    <select name="tag" class="form-control">
        ...
        <?php
        foreach ($all_project_document_tags['data'] as $tag) {
            ?>
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($project_documents['total'], $start, $per_page_items, count($project_documents['data']), 'Project Documents') ?>
    </div>
    <table class="table table-bordered table-condensed">
        ...
        <tbody>
            <?php
            foreach ($project_documents['data'] as $i => $project_document) {
                ?>
                <tr>
                    <td><?php echo $project_document['project_short_name']; ?></td>
                    <td><?php echo date('d-m-Y', strtotime($project_document['create_date'])) ?></td>
                    <td><?php echo $project_document['name']; ?></td>
                    <td><a href="<?php echo image_url($project_document['document_file']); ?>" target="_blank">Click Here</a></td>
                    <td class="tar action_column">
                        <?php if (has_permission('edit_project_document')): ?>
                            <div class="btn-group btn-group-sm">
                                <?php
                                echo linkButtonGenerator(array(
                                    'href' => build_url(array('action' => 'add_edit_project_document', 'edit' => $project_document['pk_knowledge_id'])),
                                    'action' => 'edit',
                                    'icon' => 'icon_edit',
                                    'text' => 'Edit',
                                    'title' => 'Edit Story',
                        ...
                        <?php endif; ?>
                        <?php if (has_permission('delete_project_document')): ?>
                            <div class="btn-group btn-group-sm">
                                <?php
                                echo buttonButtonGenerator(array(
                                    ...
                                    'text' => 'Delete',
                                    'title' => 'Delete Record',
                                    'attributes' => array('data-id' => $project_document['pk_knowledge_id']),
