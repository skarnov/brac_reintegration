...
if ($edit) {
    $args = array(
        'select_fields' => array(
            'district_id' => 'bd_upazilas.district_id',
            'upazila_name' => 'bd_upazilas.name AS name',
            'upazila_bn_name' => 'bd_upazilas.bn_name AS bn_name',
        ),
        'id' => $edit,
        'single' => true
    );
    $pre_data = $this->get_upazilas($args);
   
    if (!$pre_data) {
        add_notification('Invalid upazila, no data found.', 'error');
...
if ($_POST) {
    ...
    $ret = $this->add_edit_upazila($data);
    ...
    if ($ret['success']) {
        ...
        header('location: ' . url('admin/dev_location_management/manage_upazilas'));
        exit();
    } else {
        $pre_data = $_POST;
...
<div class="page-header">
    <h1><?php echo $edit ? 'Update ' : 'New ' ?>Upazila</h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            ...
            echo linkButtonGenerator(array(
                'href' => $myUrl,
                'action' => 'list',
                'text' => 'All Upazila Management',
                'title' => 'Manage Upazila Management',
                'icon' => 'icon_list',
                'size' => 'sm'
            ));
            ?>
