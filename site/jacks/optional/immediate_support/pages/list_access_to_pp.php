...
$filter_problem = $_GET['problem'] ? $_GET['problem'] : null;
...
if ($filter_problem)
    $filterString[] = 'Problem: ' . $filter_problem;
if ($filter_division)
...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'Access_To_Service_' . time() . '.csv';
    ...
    $report_title = array('', 'Access To Service', '');
    ...
    if ($data) {
        ...
        foreach ($data as $user) {
            $support_date = $user['support_date'] ? date('d-m-Y', strtotime($user['support_date'])) : 'N/A';
            $return_date = $user['return_date'] ? date('d-m-Y', strtotime($user['return_date'])) : 'N/A';
            ...
            $dataToSheet = array(
                ...
                , $user['full_name']
                , ucfirst($user['gender'])
                , ucfirst($user['disability'])
                , $user['mobile'] . "\r"
                , ucfirst($user['division'])
                , ucfirst($user['district'])
                , ucfirst($user['upazilla'])
                , ucfirst($user['permanent_city_corporation'])
                , ucfirst($user['user_union'])
                , $user['village']
                , $user['service_type'] . ', ' . $user['other_service_type']
                , ucwords(str_replace('_', ' ', $user['rescue_reason']))
                , $user['destination_country']
                , $support_date
                , $user['complain_to'] . ', ' . $user['other_complain_to']
                , ucfirst($user['service_result'])
                , $return_date
                , ucfirst($user['comment']));
    ...
    <div class="oh">
        <div class="btn-group btn-group-sm">
            ...
            echo linkButtonGenerator(array(
                ...
                'action' => 'add',
                'icon' => 'icon_add',
                'text' => 'New Access To Service',
                'title' => 'New Access To Service',
            ));
            ?>
        </div>
        <div class="btn-group btn-group-sm">
            <?php
            echo linkButtonGenerator(array(
                'href' => '?download_csv=1&name=' . $filter_name . '&gender=' . $filter_gender . '&country=' . $filter_country . '&problem=' . $filter_problem . '&city_corporation=' . $filter_city_corporation . '&division=' . $filter_division . '&district=' . $filter_district . '&sub_district=' . $filter_sub_district . '&project_id=' . $filter_project_id . '&entry_start_date=' . $filter_entry_start_date . '&entry_end_date=' . $filter_entry_end_date,
...
<div class="form-group col-sm-3">
    <label>Problem</label>
    <div class="select2-primary">
        <select class="form-control country" name="problem" style="text-transform: capitalize">
            <option value="">Select One</option>
            <?php foreach ($this->all_rescue_reasons as $key => $value) : ?>
                <option value="<?php echo $key ?>" <?php echo ($key == $filter_problem) ? 'selected' : '' ?>><?php echo $value ?></option>
            <?php endforeach ?>
        </select>
    </div>
</div>
...
<div class="table-primary table-responsive">
    ...
    <?php endif; ?>
    <div class="table-header">
        <?php echo searchResultText($results['total'], $start, $per_page_items, count($results['data']), 'Access To Service') ?>
    </div>
